 ...
 *   /api/specificatie?klant=<id>&van=2026-08-01&tot=2026-08-31
 *
...
export async function GET(request: Request) {
  ...
  else if (klantId && isGeldigeDatum(van) && isGeldigeDatum(tot) && van <= tot)
    sel = { klantId, van, tot };
  else
    return new NextResponse("Geef een factuur, of een klant met een periode op.", { status: 400 });
  ...
  const naam = factuur
    ...
    : `specificatie-${s.klant.naam.replace(/[^\w-]+/g, "_")}-${periodeVan}-${periodeTot}.pdf`;
