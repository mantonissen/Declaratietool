...
export default async function FactuurPagina({ params }: { params: Promise<{ ref: string }> }) {
  ...
  if (!s.klant || (s.uren.length === 0 && s.ritten.length === 0)) notFound();
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-5 md:px-8 md:py-8">
      ...
      <p className="label mt-2">Factuur</p>
      <h1 className="mt-1 text-2xl font-bold tracking-tight md:text-3xl">
        <span className="cijfers">{referentie}</span> · {s.klant.naam}
      </h1>
      ...
      <div className="mt-4 mb-6 flex flex-wrap items-center gap-3">
        ...
        <span className="cijfers ml-auto text-lg font-semibold">{euro(totOmzet + kmBedrag)}</span>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr>
              ...
              <th className="label border-b border-line bg-surface-2 px-3 py-2 text-left">Regel</th>
              <th className="label border-b border-line bg-surface-2 px-3 py-2 text-right">Uren</th>
              <th className="label border-b border-line bg-surface-2 px-3 py-2 text-right">Bedrag</th>
              <th className="label border-b border-line bg-surface-2 px-3 py-2 text-right"></th>
            </tr>
          </thead>
          <tbody>
            {s.uren.map((u) => (
              <tr key={u.id} className={`border-b border-line last:border-0 ${u.minuten < 0 ? "text-muted" : ""}`}>
                ...
                <td className="px-3 py-2">
                  ...
                  <span className="block text-xs text-muted">{u.project}{u.omschrijving ? ` · ${u.omschrijving}` : ""}</span>
                </td>
                <td className="cijfers px-3 py-2 text-right">{minutenAlsTijd(u.minuten)}</td>
                <td className="cijfers px-3 py-2 text-right">{euro(u.omzet)}</td>
                <td className="px-3 py-2 text-right">
                  {u.status === "gefactureerd" && !u.gecorrigeerd && u.minuten > 0 && (
      ...
      <p className="mt-2 text-xs text-muted">
        Gefactureerde regels zijn op slot. Een correctie maakt een tegenboeking in de
        huidige periode; die verschijnt in het volgende factuurvoorstel van deze klant.
      </p>
    </div>
  );
}
